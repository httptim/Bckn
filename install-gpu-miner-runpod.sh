...
# Bckn GPU Miner Installation Script for RunPod
# Optimized for containerized GPU environments
...
echo -e "${BLUE}"
echo "╔═══════════════════════════════════════════════════════════════════╗"
echo "║              Bckn GPU Miner Installer for RunPod                  ║"
...
if [ -f /.dockerenv ] || [ -n "$RUNPOD_POD_ID" ]; then
    log "Detected RunPod/container environment"
    IN_CONTAINER=true
else
    IN_CONTAINER=false
fi
...
if nvidia-smi &> /dev/null; then
    ...
    error "NVIDIA drivers not found. Please ensure GPU is properly configured in RunPod."
fi
...
log "Detected $GPU_COUNT x $GPU_NAME"

# Set GPU persistence mode (may fail in containers)
log "Attempting to set GPU persistence mode..."
nvidia-smi -pm 1 2>/dev/null || warning "Cannot set persistence mode in container. This is normal."

# Power management (often restricted in containers)
log "Checking GPU power management..."
if [[ $GPU_NAME == *"RTX 5090"* ]] || [[ $GPU_NAME == *"RTX 4090"* ]]; then
    warning "Power limit management is often restricted in RunPod containers."
    warning "The GPU will use RunPod's default power settings."
fi
...
log "Setting up mining directory..."
MINING_DIR="/workspace/bckn-mining"
mkdir -p $MINING_DIR
cd $MINING_DIR

# Download mining scripts
log "Downloading enhanced GPU miner..."
cat > bckn-miner-gpu-enhanced.py << 'MINER_SCRIPT'
#!/usr/bin/env python3
"""
...
def run_background_mode(address):
    ...
    print(f"Logs will be written to: /workspace/bckn-mining/miner.log")
    
    # Redirect output to log file
    log_file = open('/workspace/bckn-mining/miner.log', 'a')
    sys.stdout = log_file
    sys.stderr = log_file
...
chmod +x bckn-miner-gpu-enhanced.py

# Download address generator
log "Downloading address generator..."
...
#!/bin/bash
cd $MINING_DIR
screen -dmS bckn-miner python3 bckn-miner-gpu-enhanced.py $PRIVATE_KEY --background --node $NODE_URL
echo "Miner started in background. Use 'screen -r bckn-miner' to view"
EOF
...
while true; do
    ...
    echo ""
    echo "=== Recent Logs ==="
    tail -n 15 /workspace/bckn-mining/miner.log 2>/dev/null || echo "No logs yet..."
    echo ""
...
log "Creating RunPod startup script..."
cat > /workspace/on_start.sh << EOF
#!/bin/bash
# RunPod startup script for Bckn mining

echo "Starting Bckn GPU miner..."
cd $MINING_DIR
screen -dmS bckn-miner python3 bckn-miner-gpu-enhanced.py $PRIVATE_KEY --background --node $NODE_URL
echo "Miner started. Use 'screen -r bckn-miner' to view"
EOF
chmod +x /workspace/on_start.sh
...
echo "  GUI Mode:         cd $MINING_DIR && ./start-gui.sh"
echo "  Background Mode:  cd $MINING_DIR && ./start-background.sh"
echo "  Monitor:          cd $MINING_DIR && ./monitor.sh"
echo "  View Logs:        screen -r bckn-miner"
echo
echo -e "${BLUE}File Locations:${NC}"
echo "  Mining Directory: $MINING_DIR"
echo "  Log File:        $MINING_DIR/miner.log"
echo
echo -e "${YELLOW}RunPod Notes:${NC}"
echo "- The miner will auto-start when the pod restarts"
echo "- Power limits cannot be changed in RunPod containers"
echo "- Use screen to manage background sessions"
echo
